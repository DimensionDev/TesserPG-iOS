...
public struct PGPUserIDTranslator {
    ...
    public init(userID: String) {
        self.userID = userID
        (self.name, self.email, self.comment) = PGPUserIDTranslator.extractMeta(from: userID)
    }

    public init(name: String?, email: String?, comment: String?) {
        userID = PGPUserIDTranslator.buildUserID(name: name, email: email, comment: comment)
        self.name = name
        self.email = email
...
public extension PGPUserIDTranslator {
